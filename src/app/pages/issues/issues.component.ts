...
export class IssuesComponent implements OnInit {
  ...
  updateIssues(user: any, type: any) {
    ...
    if (this.assignees.length == 0) {
      ...
      this.inProgressIssues = this.issues.filter(
        (ele) => ele.status == 'DOING'
      );
      this.doneIssues = this.issues.filter((ele) => ele.status == 'DONE');
    } else {
      let filteredTodoIssues = this.issues.filter(
        (ele) => this.assignees.includes(+ele.assignee) && ele.status == 'TODO'
      );
      let filteredInProgressIssues = this.issues.filter(
        (ele) => this.assignees.includes(+ele.assignee) && ele.status == 'DOING'
      );
      let filteredDoneIssues = this.issues.filter(
        (ele) => this.assignees.includes(+ele.assignee) && ele.status == 'DONE'
      );
  ...
  drop(event: CdkDragDrop<Iissue[]>) {
    ...
    if (issueData.status !== event.container.id) {
      ...
      this._userService.updateIssue(issueData, issueData.id).subscribe(
        (data) => {},
        () => {},
        () => {
          this.isFetching = false;
